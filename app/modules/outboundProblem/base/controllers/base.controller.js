...
(function () {
  ...
  angular.module('myApp').controller("obpCtl", function ($rootScope, $scope, $state, $translate, $window, commonService, OBP_FILTER, problemOutboundBaseService) {
    // ======================================================================================================================
    function search(options, isSearchInWin){
      ...
      var grid = $("#"+ gridId).data(currentItem==="itemGroup"? "kendoTreeList": "kendoGrid"), filterItems = OBP_FILTER[currentItem] || [];
        ...
        commonService.dialogMushiny($scope.window, {
          open: function(win){
            $rootScope.deleteSure = function(){
              var rowData = grid.dataItem(rows[0]);
              problemOutboundBaseService.deleteOne(currentItem, rowData.id, function(){
                win.close();
    ...
    $rootScope.selectInWindow = function(options){
      ...
      var url = options.url || "modules/outboundProblem/base/templates/selectInWindow.html";
      commonService.dialogMushiny($scope.window, {
    ...
    // warehouse
    $rootScope.warehouseSource = problemOutboundBaseService.getDataSource({key: "getWarehouse", text: "name", value: "id"});

    // client
    $rootScope.clientSource =  problemOutboundBaseService.getDataSource({key: "getClient", text: "name", value: "id"});

    // entity_lock
    $rootScope.entityLockSource = problemOutboundBaseService.getDataSource({
