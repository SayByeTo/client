...
(function () {
  ...
  angular.module('myApp').controller("obpStationCtl", function ($scope, $rootScope, $window, $state, commonService, problemOutboundBaseService) {
    ...
    $rootScope.obpStationTypeSource = problemOutboundBaseService.getDataSource({key: "getObpStationType", text: "name", value: "id"});
    $rootScope.workstationSource = problemOutboundBaseService.getDataSource({key: "getWorkstation", text: "name", value: "id"});

    var columns = [
      {field: "name", template: "<a ui-sref='main.obpStationRead({id:dataItem.id})'>#: name # </a>", headerTemplate: "<span translate='NAME'></span>"},
      {field: "obpStationType.name",template: "<a ui-sref='main.obp_station_type'>#: obpStationType.name # </a>", headerTemplate: "<span translate='OBP_STATION_TYPE'></span>"},
      {headerTemplate: "<span translate='STATE'></span>", template: function(item){
        return item.operatorId? "BUSY": "IDLE";
      }},
      {field: "operatorId", headerTemplate: "<span translate='OPERATOR'></span>"},
      {field: "description", headerTemplate: "<span translate='DESCRIPTION'></span>"}
    ];
    $scope.obpStationGridOptions = commonService.gridMushiny({columns: columns, dataSource: problemOutboundBaseService.getGridDataSource("obpStation")});
