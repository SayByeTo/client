...
(function () {
  'use strict';
  angular.module('myApp').controller("skuToolCtl", function ($scope, $rootScope, $state, commonService, skuToolService,BACKEND_CONFIG,internalToolService) {
    ...
    $scope.goodsSKUSearch = function () {
      ...
      skuToolService.getByItemDataNo($scope.itemNo, function (v) {
        ...
        $scope.description = v.description;
        $scope.size = v.depth + "*" + v.width + "*" + v.height;
    ...
    $scope.search = function () {
      ...
      $scope.fromContainer && (data["fromContainer"] = $scope.fromContainer);
      $scope.toContainer && (data["toContainer"] = $scope.toContainer);
      $scope.operationCode && (data["operationCode"] = $scope.operationCode);
      $scope.activityCode && (data["activityCode"] = $scope.activityCode);
    ...
    var adjustColumns = [
      ...
      {field: "operationCode", headerTemplate: "<span translate='操作代码'></span>"},
      {field: "itemNo", headerTemplate: "<span translate='SKU_ID'></span>"},
      {field: "amount", headerTemplate: "<span translate='COUNT'></span>"},
      {field: "operator", headerTemplate: "<span translate='操作人'></span>"},
      {field: "source", headerTemplate: "<span translate='原始容器'></span>"},
      {field: "destination", headerTemplate: "<span translate='目的容器'></span>"},
      {field: "clientName", headerTemplate: "<span translate='CLIENT'></span>"},
      {field: "activityCode", headerTemplate: "<span translate='操作'></span>"},
      {field: "recordType", headerTemplate:"<span translate='使用工具'></span>"}];
