...
(function () {
  ...
  angular.module('myApp').controller("receivingCtl", function ($scope, $window){
  ...
  }).controller("receivingContainerCtl", function($scope, $rootScope, $state, $stateParams,receiving_commonService,receivingService,commonService){
    // ========================================================================================
      $scope.status = '0';
      $scope.scanhead = '0';
      var scan_pod = false;
    ...
    $scope.toReceiving = function(){
      ...
      $rootScope.scan_product_content_TO_INVESTIGATE=true;
        if($scope.receivingCurrent==='single'){
            $state.go('main.receivingSingle');
        }
        if($scope.receivingCurrent==='pallet'){
            alert("跳转pallet");
            $state.go('main.receivingPallet');
        }
        if($scope.receivingCurrent==='tote'){
    ...
    $scope.scanStation = function(e){
      ...
      if(keyCode != 13) return;
        receivingService.scanStation($scope.station,function (data) {
            ...
            console.log("processlist-->"+$rootScope.receiveProcessDTOList[0].desinationName);
    ...
    $scope.scanDestination = function(e){
        ...
        // $scope.destinationId = "";
        receivingService.scanDestination($scope.receiving_destination, $scope.destination, function(data){
    ...
    var columns= [
      ...
      {field: "desinationName", headerTemplate: "<span translate='DESTINATION'></span>", attributes: baseStyle},
      {field: "receiveStorageName", headerTemplate: "<span translate='CARTS_BINDING'></span>", attributes: baseStyle}
    ];
