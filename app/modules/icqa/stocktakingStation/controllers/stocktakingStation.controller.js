...
(function () {
  ...
  angular.module('myApp').controller("stocktakingStationCtl", function ($scope, $rootScope, $window, $state, commonService, ICQABaseService) {
    ...
    $rootScope.stocktakingStationTypeSource = ICQABaseService.getDataSource({key: "getStocktakingStationType", text: "name", value: "id"});
    $rootScope.workstationSource = ICQABaseService.getDataSource({key: "getWorkstation", text: "name", value: "id"});

    var columns = [
      {field: "name", template: "<a ui-sref='main.stocktakingStationRead({id:dataItem.id})'>#: name # </a>", headerTemplate: "<span translate='NAME'></span>"},
      {field: "stocktakingStationType.name",template: "<a ui-sref='main.stocktaking_station_type'>#: stocktakingStationType.name # </a>", headerTemplate: "<span translate='STOCKTAKING_STATION_TYPE'></span>"},
    ...
    $scope.stocktakingStationGridOptions = commonService.gridMushiny({columns: columns, dataSource: ICQABaseService.getGridDataSource("stocktakingStation")});
