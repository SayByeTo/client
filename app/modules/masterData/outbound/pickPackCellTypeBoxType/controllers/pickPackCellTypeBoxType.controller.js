  ...
  angular.module('myApp').controller("pickPackCellTypeBoxTypeCtl", function ($scope, commonService, masterService, pickPackCellTypeBoxTypeService){
    ...
    $scope.selectBoxTypeOptions = getComboxOption({
      change: function(){
        ...
        pickPackCellTypeBoxTypeService.getUnassignedPickPackCellTypeByBoxType(boxTypeId, function(unAssigned){
          getKendoList("listUnassignPickPackCellTypes").setDataSource(new kendo.data.DataSource({data: unAssigned}));
        });
        pickPackCellTypeBoxTypeService.getAssignedPickPackCellTypeByBoxType(boxTypeId, function(assigned){
          getKendoList("listAssignPickPackCellTypes").setDataSource(new kendo.data.DataSource({data: assigned}));
        });
      }
    });
    $scope.listAssignPickPackCellTypes = masterService.getJsonForListView({data: [], template: null/*"#:userGroup.name#,#:name#"*/});
    $scope.listUnassignPickPackCellTypes = masterService.getJsonForListView({data: [], template: null/*"#:userGroup?userGroup.name:''#,#:name#"*/});
