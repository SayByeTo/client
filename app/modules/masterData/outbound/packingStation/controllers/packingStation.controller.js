...
(function () {
  ...
  angular.module('myApp').controller("packingStationCtl", function ($scope, $rootScope, $window, commonService, masterService) {
    ...
    $rootScope.packingStationTypeSource =  masterService.getDataSource({key: "getPackingStationType", text: "name", value: "id"});
    $rootScope.workstationSource = masterService.getDataSource({key: "getWorkstation", text: "name", value: "id"});

    var columns = [
      {field: "name", template: "<a ui-sref='main.packingStationRead({id:dataItem.id})'>#: name # </a>", headerTemplate: "<span translate='NAME'></span>"},
      {field: "packingStationType.name",template: "<a ui-sref='main.packing_station_type'>#: packingStationType.name # </a>", headerTemplate: "<span translate='PACKING_STATION_TYPE'></span>"},
    ...
    $scope.packingStationGridOptions = commonService.gridMushiny({columns: columns, dataSource: masterService.getGridDataSource("packingStation")});
