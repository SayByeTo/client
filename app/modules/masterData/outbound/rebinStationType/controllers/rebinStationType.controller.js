...
(function () {
  ...
  angular.module('myApp').controller("rebinStationTypeCtl", function ($scope, $rootScope, $window, $state, commonService, masterService) {
    ...
    var columns = [
      {field: "name", template: "<a ui-sref='main.rebinStationTypeRead({id:dataItem.id})'>#: name # </a>", headerTemplate: "<span translate='NAME'></span>"},
      ...
      {width: 350, headerTemplate: "<span translate='REBIN_STATION'></span>", template: function(item){
      ...
      {width: 350, headerTemplate: "<span translate='REBIN_WALL'></span>", template: function(item){
    ...
    $scope.rebinStationTypeGridOptions = commonService.gridMushiny({columns: columns, dataSource: masterService.getGridDataSource("rebinStationType")});
