...
(function () {
  ...
  angular.module('myApp').controller("rebinStationCtl", function ($scope, $rootScope, $window,  $state, commonService, masterService) {
    ...
    $rootScope.rebinStationTypeSource = masterService.getDataSource({key: "getRebinStationType", text: "name", value: "id"});
    $rootScope.workstationSource = masterService.getDataSource({key: "getWorkstation", text: "name", value: "id"});

    var columns = [
      {field: "name", template: "<a ui-sref='main.rebinStationRead({id:dataItem.id})'>#: name # </a>", headerTemplate: "<span translate='NAME'></span>"},
      {field: "rebinStationType.name",template: "<a ui-sref='main.rebin_station_type'>#: rebinStationType.name # </a>", headerTemplate: "<span translate='REBIN_STATION_TYPE'></span>"},
    ...
    $scope.rebinStationGridOptions = commonService.gridMushiny({columns: columns, dataSource: masterService.getGridDataSource("rebinStation")});
