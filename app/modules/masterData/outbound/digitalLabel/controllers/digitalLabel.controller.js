...
(function () {
  ...
  angular.module('myApp').controller("digitalLabelCtl", function ($scope, $rootScope, $window,  $state, commonService, masterService) {
    ...
    var columns = [
      {field: "name", template: "<a ui-sref='main.digitalLabelRead({id:dataItem.id})'>#: name # </a>", headerTemplate: "<span translate='NAME'></span>"},
      {field: "labelController.name", template: "<a ui-sref='main.label_controller'>#: labelController.name # </a>", headerTemplate: "<span translate='LABEL_CONTROLLER'></span>"}
    ];
    $scope.digitalLabelGridOptions = commonService.gridMushiny({columns: columns, dataSource: masterService.getGridDataSource("digitalLabel")});
