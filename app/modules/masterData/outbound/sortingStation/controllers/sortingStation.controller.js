...
(function () {
  ...
  angular.module('myApp').controller("sortingStationCtl", function ($scope, $rootScope, $window, commonService, masterService) {
    ...
    $rootScope.sortingStationTypeSource = masterService.getDataSource({key: "getSortingStationType", text: "name", value: "id"});
    $rootScope.workstationSource = masterService.getDataSource({key: "getWorkstation", text: "name", value: "id"});

    var columns = [
      {field: "name", template: "<a ui-sref='main.sortingStationRead({id:dataItem.id})'>#: name # </a>", headerTemplate: "<span translate='NAME'></span>"},
      {field: "sortingStationType.name", template: "<a ui-sref='main.sorting_station_type'>#: sortingStationType.name # </a>", headerTemplate: "<span translate='SORTING_STATION_TYPE'></span>"},
    ...
    $scope.sortingStationGridOptions = commonService.gridMushiny({columns: columns, dataSource: masterService.getGridDataSource("sortingStation")});
