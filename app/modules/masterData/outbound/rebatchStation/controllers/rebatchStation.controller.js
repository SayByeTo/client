...
(function () {
  ...
  angular.module('myApp').controller("rebatchStationCtl", function ($scope, $rootScope, $window, commonService, masterService) {
    ...
    $rootScope.rebatchStationTypeSource = masterService.getDataSource({key: "getRebatchStationType", text: "name", value: "id"});
    $rootScope.workstationSource = masterService.getDataSource({key: "getWorkstation", text: "name", value: "id"});

    var columns = [
      {field: "name", template: "<a ui-sref='main.rebatchStationRead({id:dataItem.id})'>#: name # </a>", headerTemplate: "<span translate='NAME'></span>"},
      {field: "rebatchStationType.name", template: "<a ui-sref='main.rebatch_station_type'>#: rebatchStationType.name # </a>", headerTemplate: "<span translate='REBATCH_STATION_TYPE'></span>"},
    ...
    $scope.rebatchStationGridOptions = commonService.gridMushiny({columns: columns, dataSource: masterService.getGridDataSource("rebatchStation")});
