...
(function () {
  ...
  angular.module('myApp').controller("pickStationCtl", function ($scope, $rootScope, $window, commonService, masterService) {
    ...
    $rootScope.pickStationTypeSource = masterService.getDataSource({key: "getPickStationType", text: "name", value: "id"});
    $rootScope.workstationSource = masterService.getDataSource({key: "getWorkstation", text: "name", value: "id"});

    var columns = [
      {field: "name", template: "<a ui-sref='main.pickStationRead({id:dataItem.id})'>#: name # </a>", headerTemplate: "<span translate='NAME'></span>"},
      {field: "pickStationType.name", template: "<a ui-sref='main.pick_station_type'>#: pickStationType.name # </a>", headerTemplate: "<span translate='PICK_STATION_TYPE'></span>"},
    ...
    $scope.pickStationGridOptions = commonService.gridMushiny({columns: columns, dataSource: masterService.getGridDataSource("pickStation")});
