...
(function () {
  ...
  angular.module('myApp').controller("masterCtl", function ($rootScope, $scope, $state, $translate, $window, commonService, MASTER_FILTER, masterService) {
    // ======================================================================================================================
    function search(options, isSearchInWin){
      ...
      var grid = $("#"+ gridId).data(currentItem==="itemGroup"? "kendoTreeList": "kendoGrid"), filterItems = MASTER_FILTER[currentItem] || [];
      for(var i = 0, filters = []; i < filterItems.length; i++) {
        var filter = filterItems[i], fields = filter.field.split("."), key = fields[0], value = fields[1];
        // *****************entity_lock: start*********************
        if (["entityLock", "locale", "storageType"].indexOf(key) >= 0 && $scope[key]) {
    ...
    $rootScope.selectInWindow = function(options){
      ...
      commonService.dialogMushiny($scope.window, {
        ...
        open: function(){
          ...
          $rootScope.queryGridOptions = commonService.gridMushiny({columns: options.srcColumns, dataSource: masterService.getGridDataSource(options.srcKey), height: height-147});
