...
(function () {
  ...
  angular.module('myApp').controller("receiveStationCtl", function ($scope, $rootScope, $window, commonService, masterService) {
    ...
    $rootScope.receiveStationTypeSource = masterService.getDataSource({key: "getReceiveStationType", text: "name", value: "id"});
    $rootScope.workstationSource = masterService.getDataSource({key: "getWorkstation", text: "name", value: "id"});

    var columns = [
      {field: "name", template: "<a ui-sref='main.receiveStationRead({id:dataItem.id})'>#: name # </a>", headerTemplate: "<span translate='NAME'></span>"},
      {field: "receiveStationType.name", template: "<a ui-sref='main.receive_station_type'>#: receiveStationType.name # </a>", headerTemplate: "<span translate='RECEIVE_STATION_TYPE'></span>"},
    ...
    $scope.receiveStationGridOptions = commonService.gridMushiny({columns: columns, dataSource: masterService.getGridDataSource("receiveStation")});
