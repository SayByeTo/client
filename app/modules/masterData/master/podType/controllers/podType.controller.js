...
(function () {
  ...
  angular.module('myApp').controller("podTypeCtl", function ($scope, $rootScope, $window, $translate, commonService, masterService) {
    ...
    // 列
    var columns = [
      {field: "name", template: "<a ui-sref='main.podTypeRead({id:dataItem.id})'>#: name # </a>", headerTemplate: "<span translate='NAME'></span>"},
    ...
    $scope.podTypeGridOptions = commonService.gridMushiny({columns: columns, dataSource: masterService.getGridDataSource("podType")});

    // =================================================podTypePosition===============================================
    // podTypePosition--函数
    function binTypeEditor(container, options){
      masterService.selectEditor(container, options, masterService.getDataSource({key: "getStorageLocationType", text: "name", value: "id"}));
    }
    function dropZoneEditor(container, options){
      masterService.selectEditor(container, options, masterService.getDataSource({key: "getDropZone", text: "name", value: "id"}));
    }
    function faceEditor(container, options){
      var source = masterService.getDataSource({key: "getSelectionBySelectionKey", text: "resourceKey", value: "selectionValue", data: {selectionKey: "FACE"}});
      masterService.selectEditor(container, options, source);
    }

    // bayTypePosition--体
    var podTypePositionColumns = [
      {field: "positionNo", width: 120, editor: masterService.numberEditor, headerTemplate: "<span translate='POSITION_NO'></span>"},
      {field: "storageLocationType", width: 150, editor: binTypeEditor, headerTemplate: "<span translate='STORAGE_LOCATION_TYPE'></span>", template: function(item){
        return item.storageLocationType? item.storageLocationType.name: "";
      }},
      {field: "level", width: 60, editor: masterService.numberEditor, headerTemplate: "<span translate='LEVEL'></span>"},
      {field: "numberOfColumns", width: 60, editor: masterService.numberEditor, headerTemplate: "<span translate='NUMBER_OF_COLUMNS'></span>"},
      {field: "dropZone", width: 120, editor: dropZoneEditor, headerTemplate: "<span translate='DROP_ZONE'></span>", template: function(item){
        return item.dropZone? item.dropZone.name: "";
      }},
      {field: "face", width: 100, editor: faceEditor, headerTemplate: "<span translate='FACE'></span>", template: function(item){
  ...
  }).controller("podTypeCreateCtl", function ($scope, $state, masterService){
