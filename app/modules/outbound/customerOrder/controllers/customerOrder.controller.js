...
(function () {
  ...
  angular.module('myApp').controller("customerOrderCtl", function ($scope, $window, $rootScope, commonService, outboundService, customerOrderService) {
    ...
    // 列
    var columns = [
      {field: "orderNo", template: "<a ui-sref='main.customerOrderRead({id:dataItem.id})'>#: orderNo # </a>", headerTemplate: "<span translate='ORDER_NO'></span>"},
      ...
      //}},
      {field: "strategy.name", headerTemplate: "<span translate='ORDER_STRATEGY'></span>"}
    ];
    $scope.customerOrderGridOptions = commonService.gridMushiny({dataSource:outboundService.getGridDataSource("customerOrder"), columns:columns});
    ...
    var customerOrderPositionColumns = [
      {field: "positionNo", width: 60, editor: outboundService.numberEditor, headerTemplate: "<span translate='POSITION_NO'></span>"},
      {field: "itemData", width: 180, headerTemplate: "<span translate='ITEM_DATA'></span>", editor: itemDataEditor, template: function(item){
        return item.itemData? item.itemData.name: "";
      }},
      {field: "amount", width: 60, editor: outboundService.numberEditor, headerTemplate: "<span translate='AMOUNT'></span>"},
  ...
  }).controller("customerOrderCreateCtl", function ($scope, $rootScope, $state, outboundService) {
    $scope.client = {};
    ...
    $scope.validate = function (event) {
      ...
      if ($scope.validator.validate()) {
        ...
        outboundService.create("customerOrder", {
          ...
          "deliveryDate": $scope.sortCode? $scope.sortCode.exsdTime: "",
