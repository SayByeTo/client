...
(function () {
  ...
  angular.module('myApp').controller("rebatchCtl", function ($scope, $rootScope, $state, commonService, outboundService, reBatchService) {
    ...
    $scope.inputImformation = function (e) {
      ...
      if (keycode == 13) {
        ...
        if (step == 1) {
          ...
          reBatchService.scanReBatchContainer($scope.imformation, function (data) {
          ...
          if ($scope.imformation == $scope.slotNumber) {
            var data={id:$scope.id, rebatchSlotName: $scope.slotNumber, rebatchStationId:$scope.stationId};
    ...
    $scope.okClick = function (reBatchSlotName,requestId) {
      reBatchService.doReBatch(requestId,function () {
          reBatchService.getReBatchInfo(function (datas) {
            initGrid(datas)
          });
      })
