...
(function () {
  ...
  angular.module('myApp').controller("ibpCtl", function ($rootScope, $scope, $state, $translate, $window, commonService, IBP_FILTER, problemInboundBaseService) {
    ...
    function search(options, isSearchInWin){
      ...
      var grid = $("#"+ gridId).data(currentItem==="itemGroup"? "kendoTreeList": "kendoGrid"), filterItems = IBP_FILTER[currentItem] || [];
    ...
    $rootScope.selectInWindow = function(options){
      ...
      commonService.dialogMushiny($scope.window, {
        ...
        open: function(){
          ...
          $rootScope.queryGridOptions = commonService.gridMushiny({columns: options.srcColumns, dataSource: problemInboundBaseService.getGridDataSource(options.srcKey), height: height-147});
    ...
    // warehouse
    $rootScope.warehouseSource = problemInboundBaseService.getDataSource({key: "getWarehouse", text: "name", value: "id"});

    // client
    $rootScope.clientSource =  problemInboundBaseService.getDataSource({key: "getClient", text: "name", value: "id"});
