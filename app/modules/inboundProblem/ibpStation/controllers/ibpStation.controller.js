...
(function () {
  ...
  angular.module('myApp').controller("ibpStationCtl", function ($scope, $rootScope, $window, $state, commonService, problemInboundBaseService) {
    ...
    $rootScope.ibpStationTypeSource = problemInboundBaseService.getDataSource({key: "getIbpStationType", text: "name", value: "id"});
    $rootScope.workstationSource = problemInboundBaseService.getDataSource({key: "getWorkstation", text: "name", value: "id"});

    var columns = [
      {field: "name", template: "<a ui-sref='main.ibpStationRead({id:dataItem.id})'>#: name # </a>", headerTemplate: "<span translate='NAME'></span>"},
      {field: "ibpStationType.name",template: "<a ui-sref='main.ibp_station_type'>#: ibpStationType.name # </a>", headerTemplate: "<span translate='IBP_STATION_TYPE'></span>"},
      {field: "description", headerTemplate: "<span translate='DESCRIPTION'></span>"}
    ];
    $scope.ibpStationGridOptions = commonService.gridMushiny({columns: columns, dataSource: problemInboundBaseService.getGridDataSource("ibpStation")});
